...
WeatherData parseJson(char *json)
{
    ...
    if (ts)
        strlcpy(data.update_ts, ts, sizeof(data.update_ts));
    else
        strcpy(data.update_ts, "Unknown");
    ...
    if (cur_ts)
        strlcpy(data.current.timestamp, cur_ts, sizeof(data.current.timestamp));

    data.current.temp = cur["temp"] | -99.0;
    data.current.rain = cur["rain"] | -99.0;
    data.current.cloudCover = cur["cloudco"] | -99;
    data.current.windSpeed60 = cur["wind60"] | -99.0;
    data.current.dew_point = cur["dew_point"] | -99.0;
    const char *cur_icon = cur["icon"];

    if (cur_icon)
        strlcpy(data.current.icon, cur_icon, sizeof(data.current.icon));
    else
        strcpy(data.current.icon, "unknown");
    ...
    else
    {
        ...
        for (JsonObject f : arr)
        {
            ...
            if (f_ts)
                strlcpy(data.forecast[count].timestamp, f_ts, sizeof(data.forecast[count].timestamp));

            data.forecast[count].temp = f["temp"] | -99.0;
            data.forecast[count].rain_probability = f["rain_probability"] | -99;
            data.forecast[count].cloudCover = f["cloudco"] | -99;
            data.forecast[count].windSpeed = f["wind"] | -99.0;
            data.forecast[count].dew_point = f["dew_point"] | -99.0;

            const char *f_icon = f["icon"];
            if (f_icon)
                strlcpy(data.forecast[count].icon, f_icon, sizeof(data.forecast[count].icon));
            else
                strcpy(data.forecast[count].icon, "unknown");
